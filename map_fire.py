...
# ── Пути ────────────────────────────────────────────────────────────────────
DATA_DIR    = Path(r"C:\Users\Пользователь\Documents\диплом\model")
# Используем предфильтрованный файл (запустите prepare_map_data.py один раз)
PRED_FILE   = DATA_DIR / "model_output" / "predictions_primorsky.csv"
BORDER_FILE = DATA_DIR / "primorsky_coords.csv"
...
def load_predictions(path: Path) -> pd.DataFrame:
    """Загружает весь файл прогнозов один раз."""
    if not path.exists():
